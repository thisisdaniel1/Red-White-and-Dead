...
public class Soldier : MonoBehaviour
{
    ...
    public void Initialize(Unit u, MeleeStatsHolder stats, Vector3 targetPos, Vector3 targetLookAt)
    {
    ...
    //{
    //    if (collision.gameObject.layer == gameObject.layer && collision.gameObject.GetComponent<Soldier>().unit != unit && !unit.isInFight)
    //    {
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        var otherUnit = collision.gameObject.GetComponent<Soldier>().unit;
    ...
    private void OnCollisionStay(Collision collision)
    {
        ...
        if (otherUnit != unit && !unit.isInFight && !otherUnit.isInFight)
        {
            rb.AddForce(Random.Range(2.5f,5f) * movementForce * Vector3.RotateTowardstargetPos - transform.position, GetRandomVectorXZ(1), 60 * Mathf.Deg2Rad, 100).normalized;
            transform.LookAt(unit.transform.forward);
        }
    }
